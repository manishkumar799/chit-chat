import React from "react";

function Card() {
  return (
    <>
      <div
        className="flex flex-col gap-2 "
        style={{
          maxHeight: "59vh",
          overflowY: "auto",
          scrollbarWidth: "none",
          msOverflowStyle: "none",
        }}
      >
        {Array.from({ length: 30 }, (_, i) => (
          <div
            className="h-[60px] px-2 flex items-center gap-2 shadow-md bg-[#efeeee] p-2 custom-shadow border-2 border-blue-200"
            key={i}
          >
            <div className="flex-start ">
              <img
                className="h-[50px] w-[50px] rounded-full border-x-2"
                src="https://cdn-icons-png.flaticon.com/512/21/21104.png"
                alt=""
              />
            </div>
            <div className="flex ">
              <div>
                <div className="font-bold">Test User</div>
                <div className="text-sm">Lorem ipsum dolor sit!</div>
              </div>
              <div className="flex flex-col justify-center items-center">
                <div className="text-[12px] self-start">03:00PM</div>
                <div className="text-[15px] rounded-full w-5 h-5 flex justify-center items-center border-2 border-black">
                  3
                </div>
              </div>
            </div>
          </div>
        ))}
      </div>
    </>
  );
}

export default Card;
